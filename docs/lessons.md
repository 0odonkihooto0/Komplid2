...
**`migrate resolve --applied` нельзя добавлять для НОВЫХ миграций в scripts/start.sh.**
`migrate resolve --applied <name>` помечает миграцию как «уже выполнена» БЕЗ запуска SQL.
Если добавить свежесозданную миграцию в этот список — `migrate deploy` её пропустит → таблица/колонка не создастся в проде → 500 ошибки в рантайме.
Список `--applied` в start.sh ТОЛЬКО для миграций, ранее применённых через `db push` (схема есть в БД, но нет в `_prisma_migrations`).
Новые миграции (созданные через `migrate dev` или вручную) — не добавлять в `--applied`; `migrate deploy` применит их сам на первом деплое.
