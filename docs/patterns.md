...
### Канонический путь API для объекта строительства

Все API-операции над сущностью `BuildingObject` и её дочерними коллекциями живут только
по пути `/api/projects/[projectId]/*`. Никаких параллельных `/api/objects/[objectId]/*` —
папка удалена полностью, восстанавливать её запрещено.

UI-страницы остаются на `/objects/[objectId]/*` — это сознательный рассинхрон между URL
фронта и API:
- FK в Prisma называется `projectId` (переименование сломает миграции и `$queryRaw`).
- Исторически UI-сегмент `objects/` появился при ребрендинге «Проект» → «Объект строительства»
  и остался ради стабильных закладок пользователей.

В клиентском коде `{ params }: { params: { objectId: string } }` (страницы, компоненты) — это
имя **UI-параметра**, значение совпадает с `projectId` API. Передавать его в fetch как
`${objectId}` в строку `/api/projects/${objectId}/...` — нормально: значение то же самое.
...
│   │   │   ├── objects/             # Список объектов (единственный UI-корень)
...
│   │   │   │       └── reports/     # Отчёты
│   │   │   │   # старый /projects/* больше не существует — HTTP 308 на /objects/*
│   │   │   │   # настроен в next.config.mjs → redirects()
│   │   │   ├── organizations/
│   │   │   ├── documents/
│   │   │   ├── analytics/
...
* **`$queryRaw` / `$executeRaw` — имена таблиц и колонок не транслируются Prisma**: raw SQL идёт напрямую в PostgreSQL. Таблицы: использовать имя из `@@map("snake_table")`, не PascalCase модели. Колонки: без `@map` Prisma создаёт колонки в camelCase (`projectId`, `createdAt`) — в SQL обязательно в кавычках: `"projectId"`. PostgreSQL складывает unquoted идентификаторы в lowercase (`project_id` ≠ `projectId`). Исключение: колонки, созданные вручную через ALTER TABLE (например, `search_vector`).
* **`<SelectItem value="">` запрещён в Radix UI v2.2.6+**: любой `<SelectItem value="">` — включая `disabled` — вызывает runtime-ошибку «A \<Select.Item /> must have a value prop that is not an empty string». Использовать сентинелы: `value="ALL"` для фильтра «Все», `value="NONE"` для необязательного поля «Не указан/Без привязки», `value="__PLACEHOLDER__"` для disabled-заглушек. Паттерн адаптера: `<Select value={state || 'ALL'} onValueChange={(v) => setState(v === 'ALL' ? '' : v)}>` — внутренний стейт остаётся `''`, логика `state || undefined` и `if (state)` не нарушается.
