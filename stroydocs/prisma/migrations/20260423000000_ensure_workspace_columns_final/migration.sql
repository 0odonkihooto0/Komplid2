-- Финальная идемпотентная миграция: гарантированное восстановление workspace-структур
--
-- Причина: две предыдущие rescue-миграции (#60000 и #22000000) были помечены
-- как applied в _prisma_migrations без выполнения SQL из-за бага max_attempts=55.
-- Таблица workspaces существовала → check-migration-integrity.js пропускал проверку →
-- _prisma_migrations не сбрасывался → migrate deploy видел их как applied → пропускал.
-- Результат: users.activeWorkspaceId отсутствует в БД → P2022 при регистрации.
--
-- Данная миграция (#98) — НОВАЯ запись, которой нет в _prisma_migrations → будет выполнена.
-- Все операции полностью идемпотентны: IF NOT EXISTS / DO $$ EXCEPTION WHEN duplicate_*.
