...
async function resolveIndicator(objectId: string, indicatorId: string, organizationId: string) {
  return db.projectIndicator.findFirst({
    where: {
      id: indicatorId,
      project: { id: objectId, organizationId },
    },
  });
}

// PATCH /api/objects/[objectId]/project-indicators/[indicatorId]
export async function PATCH(
  req: NextRequest,
  { params }: { params: { objectId: string; indicatorId: string } }
) {
  try {
    const session = await getSessionOrThrow();
    const { organizationId } = session.user;

    const indicator = await resolveIndicator(params.objectId, params.indicatorId, organizationId);
...
// DELETE /api/objects/[objectId]/project-indicators/[indicatorId]
