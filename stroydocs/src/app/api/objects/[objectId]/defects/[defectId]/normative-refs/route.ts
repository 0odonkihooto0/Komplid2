...
async function getDefectOrThrow(defectId: string, objectId: string, orgId: string) {
...
// GET /api/objects/[objectId]/defects/[defectId]/normative-refs — список ссылок
export async function GET(_req: NextRequest, { params }: { params: Params }) {
  try {
    const session = await getSessionOrThrow();
    const defect = await getDefectOrThrow(params.defectId, params.objectId, session.user.organizationId);
...
// POST /api/objects/[objectId]/defects/[defectId]/normative-refs — добавить ссылку
