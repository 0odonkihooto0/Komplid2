...
const DEFECT_INCLUDE = {
  ...
  inspection: {
    select: { id: true, number: true, status: true, startedAt: true, completedAt: true },
  },
  prescription: {
...
export async function PATCH(req: NextRequest, { params }: { params: Params }) {
  try {
    ...
    const existing = await db.defect.findFirst({
      where: { id: params.defectId, projectId: params.objectId, buildingObject: { organizationId: orgId } },
      select: { id: true },
    });
    ...
    await invalidateAnalyticsCache(params.objectId, orgId);
