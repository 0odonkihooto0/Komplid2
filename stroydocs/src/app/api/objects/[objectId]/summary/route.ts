...
export async function GET(_req: NextRequest, { params }: { params: Params }) {
  try {
    ...
    const cacheKey = `summary:object:${objectId}`;
    ...
    const data = await getCachedAnalytics(cacheKey, async () => {
      const [
        ...
        db.ganttTask.findMany({
          where: { version: { isActive: true, projectId: objectId } },
        ...
        db.prescription.groupBy({
          by: ['status'],
          where: { inspection: { projectId: objectId } },
          _count: { id: true },
          orderBy: { status: 'asc' },
        }),
        ...
        db.executionDoc.groupBy({
          by: ['status'],
          where: { contract: { projectId: objectId } },
          _count: { id: true },
          orderBy: { status: 'asc' },
        }),
        ...
        db.designDocument.count({
          where: { projectId: objectId, isDeleted: false, status: { not: 'CANCELLED' } },
        }),
