  ...
  try {
    ...
    const [
      ...
      db.designDocument.groupBy({
        by: ['status'],
        where: { projectId: params.objectId, isDeleted: false },
        _count: { id: true },
      }),

      // Виджет 2: Документов по статусам согласования (через back-relation designDoc)
      db.approvalRoute.findMany({
        where: { designDoc: { projectId: params.objectId, isDeleted: false } },
        select: { status: true },
      }),
      ...
      db.designDocComment.groupBy({
        by: ['status'],
        where: { doc: { projectId: params.objectId, isDeleted: false } },
        _count: { id: true },
      }),
      ...
      db.designTaskComment.groupBy({
        by: ['status'],
        where: { task: { projectId: params.objectId } },
        _count: { id: true },
      }),
      ...
      db.designDocComment.groupBy({
        by: ['assigneeId'],
        where: {
          doc: { projectId: params.objectId, isDeleted: false },
          assigneeId: { not: null },
        },
        _count: { id: true },
        orderBy: { _count: { id: 'desc' } },
