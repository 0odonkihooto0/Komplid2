...
async function verifyAccess(objectId: string, cameraId: string, organizationId: string) {
  ...
  const camera = await db.videoCamera.findFirst({
    where: { id: cameraId, projectId: objectId },
  });
...
export async function PATCH(
  req: NextRequest,
  { params }: { params: { objectId: string; cameraId: string } }
) {
  try {
