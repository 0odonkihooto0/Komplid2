  ...
  try {
    ...
    const [
      ...
      db.ganttTask.findMany({
        where: {
          version: { isActive: true, projectId: projectId },
    ...
    if (pirStageIds.length > 0) {
      const [pirTasks, pirTasksToday] = await Promise.all([
        db.ganttTask.findMany({
          where: {
            version: { isActive: true, projectId: projectId, stageId: { in: pirStageIds } },
            parentId: null,
          },
    ...
    if (smrStageIds.length > 0) {
      const [smrTasks, smrTasksToday] = await Promise.all([
        db.ganttTask.findMany({
          where: {
            version: { isActive: true, projectId: projectId, stageId: { in: smrStageIds } },
            parentId: null,
          },
          select: { amount: true },
        }),
