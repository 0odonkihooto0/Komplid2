...
type Params = { params: { projectId: string; id: string } };
...
async function verifyAccess(projectId: string, packageId: string, orgId: string) {
  ...
  const pkg = await db.idClosurePackage.findFirst({
    where: { id: packageId, projectId: projectId },
    include: {
      createdBy: {
...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const result = await verifyAccess(params.projectId, params.id, session.user.organizationId);
