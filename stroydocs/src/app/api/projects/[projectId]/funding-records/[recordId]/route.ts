...
async function verifyAccess(projectId: string, recordId: string, organizationId: string) {
  ...
  const record = await db.fundingRecord.findFirst({
    where: { id: recordId, projectId: projectId },
  });
...
export async function PATCH(
  req: NextRequest,
  { params }: { params: { projectId: string; recordId: string } }
) {
  try {
