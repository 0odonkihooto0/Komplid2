...
export async function PATCH(
  req: NextRequest,
  { params }: { params: { projectId: string; contractId: string; versionId: string; taskId: string } },
) {
  try {
    ...
    // Пересчитать критический путь после изменения дат
    ...
    logger.error({ err: error }, 'Ошибка обновления задачи графика');
...
export async function DELETE(
  ...
  try {
    ...
    // Каскадно удалить детей
    await deleteTaskRecursive(params.taskId);

    return successResponse({ ok: true });
  } catch (error) {
    if (error instanceof NextResponse) return error;
    logger.error({ err: error }, 'Ошибка удаления задачи графика');
    return errorResponse('Внутренняя ошибка сервера', 500);
  }
}
