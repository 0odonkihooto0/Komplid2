...
import { Plus, AlertTriangle, Clock, CheckCircle2, Download, MoreHorizontal } from 'lucide-react';
...
import { useDefects, useDeleteDefect, useChangeDefectStatus } from '@/components/modules/defects/useDefects';
...
function DefectStatusBadge({ status }: { status: string }) {
  return (
    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_STYLES[status] ?? 'bg-gray-100 text-gray-600'}`}>
...
export function DefectsContent({ projectId, embedded }: Props) {
  const [createOpen, setCreateOpen] = useState(false);
  const [statusFilter, setStatusFilter] = useState('');
  const [categoryFilter, setCategoryFilter] = useState('');
  const [overdueOnly, setOverdueOnly] = useState(false);

  const { data, isLoading } = useDefects(projectId, {
  ...
  const overdue = defects.filter(
    (d) => d.deadline && new Date(d.deadline) < now && (d.status === 'OPEN' || d.status === 'IN_PROGRESS'),
  ).length;
  ...
  return (
    <div className="space-y-5">
      {!embedded && (
        <PageHeader title="Дефектовка" description="Реестр дефектов и замечаний стройконтроля" />
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Label htmlFor="overdue" className="cursor-pointer text-sm">Только просроченные</Label>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">Дефекты не найдены.</p>
      ) : (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-32">Договор</TableHead>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          {defect.status !== 'IN_PROGRESS' && (
                            <DropdownMenuItem
                              onClick={() =>
                                changeStatus.mutate({ defectId: defect.id, status: 'IN_PROGRESS' as DefectStatus })
                              }
                            >
                              В работе
                            </DropdownMenuItem>
                          ...
                          {defect.status !== 'RESOLVED' && (
                            <DropdownMenuItem
                              onClick={() =>
                                changeStatus.mutate({ defectId: defect.id, status: 'RESOLVED' as DefectStatus })
                              }
                            >
                              Устранён
                            </DropdownMenuItem>
                          ...
                          {defect.status !== 'CONFIRMED' && (
                            <DropdownMenuItem
                              onClick={() =>
                                changeStatus.mutate({ defectId: defect.id, status: 'CONFIRMED' as DefectStatus })
                              }
                            >
                              Подтверждён
                            </DropdownMenuItem>
                          ...
                          <DropdownMenuItem
                            ...
                            onClick={() => {
                              if (confirm('Удалить дефект?')) deleteDefect.mutate(defect.id);
                            }}
                          >
                            Удалить
                          </DropdownMenuItem>
