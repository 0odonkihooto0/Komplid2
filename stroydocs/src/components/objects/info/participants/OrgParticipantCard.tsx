...
export function OrgParticipantCard({
  participant,
  onAddRole,
  onDeleteRole,
  onDelete,
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border bg-card p-3 shadow-sm">
      {/* Шапка: иконка + название + ИНН */}
      <div className="flex items-start gap-2">
        <Building2 className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium">{participant.organization.name}</p>
          <p className="text-xs text-muted-foreground">ИНН: {participant.organization.inn}</p>
        </div>
        {/* Кнопки действий */}
        <div className="flex shrink-0 gap-1">
          <Button variant="ghost" size="icon" className="h-7 w-7" aria-label="Вложения" disabled>
      ...
      <ConfirmDialog
        ...
        description={`«${participant.organization.name}» будет удалена из списка участников. Все роли будут удалены.`}
