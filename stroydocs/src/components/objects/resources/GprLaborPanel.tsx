...
import { useGanttVersions, useGprResources, type GprLaborItem } from './usePlanning';
import { useToast } from '@/hooks/useToast';

// ─── Колонки таблицы ─────────────────────────────────────────────────────────

const columns: ColumnDef<GprLaborItem>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-sm text-right block font-medium">
        {row.original.manHours.toLocaleString('ru-RU', { maximumFractionDigits: 2 })}
      </span>
    ),
  },
  {
...
interface GprLaborPanelProps {
  objectId: string;
}

export function GprLaborPanel({ objectId }: GprLaborPanelProps) {
  ...
  const { items, total, isLoading } = useGprResources<GprLaborItem>(objectId, {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold">Рабочая сила и кадры из ГПР</h3>
      ...
      {!versionId ? (
        <div className="h-40 flex items-center justify-center text-sm text-muted-foreground border rounded-md">
          Выберите версию ГПР для отображения трудовых ресурсов
        </div>
      ) : isLoading ? (
        ...
        <div className="h-40 flex items-center justify-center text-sm text-muted-foreground border rounded-md">
          В выбранной версии ГПР нет задач с человеко-часами
        </div>
      ) : (
        <DataTable
          columns={columns}
