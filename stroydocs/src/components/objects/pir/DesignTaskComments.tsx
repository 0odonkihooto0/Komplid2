...
export function DesignTaskComments({ projectId, taskId, sessionUserId }: Props) {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ['design-task-comments', taskId] });

  const { data: comments = [], isLoading } = useQuery<DesignTaskComment[]>({
    queryKey: ['design-task-comments', taskId],
    queryFn: async () => {
      const res = await fetch(baseUrl);
      if (!res.ok) throw new Error('Ошибка загрузки замечаний');
      const json = await res.json();
      return (json.data as { data: DesignTaskComment[] })?.data ?? [];
    },
    enabled: !!taskId,
  });
  ...
  const patchMutation = useMutation({
    mutationFn: async (payload: Record<string, unknown>) => {
      ...
      const json = await res.json();
      return json.data as DesignTaskComment;
    },
    onSuccess: (updated) => {
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 line-clamp-2 text-sm">{c.description}</p>
                  {c.assignee && (
            ...
            <>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm">{selectedComment.description}</p>
                </div>
