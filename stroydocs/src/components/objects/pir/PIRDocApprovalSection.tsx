...
import type { DocApprovalRoute } from './useDesignDocDetail';
...
export function PIRDocApprovalSection({ projectId, docId, approvalRoute, docStatus }: Props) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const workflowUrl = `/api/objects/${projectId}/design-docs/${docId}/workflow`;
  ...
  const startMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        ...
        throw new Error((err as { error?: string }).error ?? 'Ошибка запуска согласования');
      }
    },
    onSuccess: () => {
  ...
  const isTerminal = docStatus === 'APPROVED' || docStatus === 'CANCELLED';
    ...
    <div className="space-y-4 rounded-md border p-4">
      ...
      {!approvalRoute && (
        <p className="text-sm text-muted-foreground">
          Маршрут согласования не запущен. Нажмите «Запустить», чтобы отправить документ на согласование.
        </p>
      )}
