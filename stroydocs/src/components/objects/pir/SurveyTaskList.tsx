...
export function SurveyTaskList({ objectId, projectId }: Props) {
  const router = useRouter();
  const [dialogOpen, setDialogOpen] = useState(false);
  const { tasks, isLoading } = useDesignTasks(projectId, 'SURVEY');

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold">Задания на изыскания</h2>
            ...
            {!isLoading && tasks.length === 0 && (
              <tr>
                <td colSpan={6} className="px-3 py-8 text-center text-sm text-muted-foreground">
                  Нет заданий на изыскания. Создайте первое.
                </td>
              </tr>
            )}
              ...
              tasks.map((task) => {
                ...
                return (
                  <tr
                    ...
                    onClick={() =>
                      router.push(`/objects/${objectId}/pir/survey-task/${task.id}`)
                    }
                  >
