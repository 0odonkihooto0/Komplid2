...
import { useUpdateTask, useDeleteTask } from './useGanttTasks';
...
export function GanttTaskPanel({ task, projectId, contractId, versionId, onClose, onExecDocsOpen }: Props) {
  const [deleteOpen, setDeleteOpen] = useState(false);
  const updateTask = useUpdateTask(projectId, contractId, versionId);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- форма сбрасывается только при смене задачи (task.id), не при каждом обновлении полей с сервера
  }, [task.id, reset]);
