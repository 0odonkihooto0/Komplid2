...
export function Sidebar() {
  ...
  if (!mounted) {
    return <aside className="flex h-screen w-60 flex-col border-r bg-[#1e2d4f]" />;
  }

  return (
    <aside
      className={cn(
        'relative flex h-screen flex-col border-r border-r-[#2d3f63] bg-[#1e2d4f] transition-all duration-200',
        isCollapsed ? 'w-14' : 'w-60'
      )}
    >
      {/* Шапка с логотипом и кнопкой свёртывания */}
      <div className={cn('flex h-14 items-center border-b border-b-[#2d3f63]', isCollapsed ? 'justify-center px-0' : 'justify-between px-4')}>
        {isCollapsed ? (
        ...
        <Button
          variant="ghost"
          size="icon"
          className={cn('h-7 w-7 flex-shrink-0 text-white/70 hover:text-white hover:bg-white/[0.08]', isCollapsed && 'ml-0')}
          onClick={toggle}
